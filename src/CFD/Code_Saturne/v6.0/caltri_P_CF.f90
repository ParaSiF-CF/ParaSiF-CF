...
!===============================================================================
! MUI local variables setup
integer    ::    nbsubcpl = 1
save       nbsubcpl
integer          ii
logical    ::    hasInitialised = .false.
save          hasInitialised
...
save             fr, ak, IQNILS
integer          cont
integer    ::    italimTemp = 1
integer          ilelt, nlelt, rankMUI, sizeMUI
integer, allocatable, dimension(:) :: lstelt
double precision coord_min_sendX, coord_min_sendY, coord_min_sendZ
double precision coord_max_sendX, coord_max_sendY, coord_max_sendZ
double precision coord_min_rcvX, coord_min_rcvY, coord_min_rcvZ
double precision coord_max_rcvX, coord_max_rcvY, coord_max_rcvZ
integer :: iii, iiiTemp, isize, iindex, isizeIndex
integer :: barrier
logical :: skip
double precision, dimension(:,:), allocatable :: clist
double precision, dimension(:), allocatable :: cindex
! MUI local variables end
!===============================================================================
...
interface
  ...
  ( nvar   , nscal  ,                                              &
    ...
    subroutine tridim(itrale, nvar, nscal, dt, sub_iteration_MUI_Coupling, &
                    nbsubcpl, init_und_relx_coupling, push_force_MUI, &
                    push_in_C, fr, ak, IQNILS, muiCouplingMethod)
    ...
    !===============================================================================
    ! MUI local variables setup
    integer    ::    sub_iteration_MUI_Coupling, nbsubcpl
    double precision init_und_relx_coupling
    logical    ::    push_force_MUI
    logical    ::    push_in_C
    type(muiCouplingFixedRelaxation) :: fr
    type(muiCouplingAitken) :: ak
    type(muiCouplingIQNILS) :: IQNILS
    integer          muiCouplingMethod
    ! MUI local variables end
    !===============================================================================
...
if (hasInitialised .eqv. .false.) then
    cont = 0

    coord_min_sendX = 1e10
    coord_min_sendY = 1e10
    coord_min_sendZ = 1e10

    coord_max_sendX =-1e10
    coord_max_sendY =-1e10
    coord_max_sendZ =-1e10
    ...
    do ilelt = 1, nlelt
      ...
      if (coord_min_sendX .GE. cdgfbo(1, ifac)) then
        coord_min_sendX = cdgfbo(1, ifac)
      endif
      if (coord_min_sendY .GE. cdgfbo(2, ifac)) then
        coord_min_sendY = cdgfbo(2, ifac)
      endif
      if (coord_min_sendZ .GE. cdgfbo(3, ifac)) then
        coord_min_sendZ = cdgfbo(3, ifac)
      endif

      if (coord_max_sendX .LE. cdgfbo(1, ifac)) then
        coord_max_sendX = cdgfbo(1, ifac)
      endif
      if (coord_max_sendY .LE. cdgfbo(2, ifac)) then
        coord_max_sendY = cdgfbo(2, ifac)
      endif
      if (coord_max_sendZ .LE. cdgfbo(3, ifac)) then
        coord_max_sendZ = cdgfbo(3, ifac)
      endif
    ...
    call mui_announce_span_send(coord_min_sendX,  &
                                coord_min_sendY,  &
                                coord_min_sendZ,  &
                                coord_max_sendX,  &
                                coord_max_sendY,  &
                                coord_max_sendZ)
    if ((coord_min_sendX > coord_max_sendX).or.(coord_min_sendY > coord_max_sendY).or.(coord_min_sendZ > coord_max_sendZ)) then

        local_push = .false.

    endif
    
    write(nfecra, *) "{CS} coupling point size to be passed to FSI coupling Method: ", cont

    call getfbr(fetch_MUI_name, nlelt, lstelt)
    ...
    do ilelt = 1, nlelt
      ifac = lstelt(ilelt)
      do ii = ipnfbr(ifac), ipnfbr(ifac+1)-1
        inod = nodfbr(ii) !! index-number iith node of face ifac.

          iindex = iindex+1

          if (coord_min_rcvX .GE. xyzno0(1, inod)) then
            coord_min_rcvX = xyzno0(1, inod)
          endif
          if (coord_min_rcvY .GE. xyzno0(2, inod)) then
            coord_min_rcvY = xyzno0(2, inod)
          endif
          if (coord_min_rcvZ .GE. xyzno0(3, inod)) then
            coord_min_rcvZ = xyzno0(3, inod)
          endif

          if (coord_max_rcvX .LE. xyzno0(1, inod)) then
            coord_max_rcvX = xyzno0(1, inod)
          endif
          if (coord_max_rcvY .LE. xyzno0(2, inod)) then
            coord_max_rcvY = xyzno0(2, inod)
          endif
          if (coord_max_rcvZ .LE. xyzno0(3, inod)) then
            coord_max_rcvZ = xyzno0(3, inod)
          endif

          if(allocated(fetchDataArray)) then
              skip=.false.
              iiiTemp = -99

              isize = size(fetchDataArray)/6
              allocate(clist(isize+1,6))
              isizeIndex = size(fetchIndexArray)
              allocate(cindex(isizeIndex+1))

              do iii=1,isizeIndex          
                  cindex(iii) = fetchIndexArray(iii)
              end do

              do iii=1,isize          
                  clist(iii,1) = fetchDataArray(iii,1)
                  clist(iii,2) = fetchDataArray(iii,2)
                  clist(iii,3) = fetchDataArray(iii,3)
                  clist(iii,4) = fetchDataArray(iii,4)
                  clist(iii,5) = fetchDataArray(iii,5)
                  clist(iii,6) = fetchDataArray(iii,6)
                  if ((fetchDataArray(iii,1) .eq. xyzno0(1, inod))  &
                        .and.(fetchDataArray(iii,2) .eq. xyzno0(2, inod))  &
                        .and.(fetchDataArray(iii,3) .eq. xyzno0(3, inod))) then
                    skip=.true.
                    iiiTemp = iii
                    EXIT
                  endif
              ...
              if (skip .eqv. .false.) then
                  clist(isize+1,1) = xyzno0(1, inod)
                  clist(isize+1,2) = xyzno0(2, inod)
                  clist(isize+1,3) = xyzno0(3, inod)
                  clist(isize+1,4) = 0.0
                  clist(isize+1,5) = 0.0
                  clist(isize+1,6) = 0.0
                  deallocate(fetchDataArray)
                  call move_alloc(clist, fetchDataArray)
                  cindex(isizeIndex+1)=iindex
                  deallocate(fetchIndexArray)
                  call move_alloc(cindex, fetchIndexArray)
              else
                  deallocate(clist)
                  cindex(isizeIndex+1)=iiiTemp
                  deallocate(fetchIndexArray)
                  call move_alloc(cindex, fetchIndexArray)
              endif
          else
              allocate(fetchDataArray(1,6))
              fetchDataArray(1,1) = xyzno0(1, inod)
              fetchDataArray(1,2) = xyzno0(2, inod)
              fetchDataArray(1,3) = xyzno0(3, inod)
              fetchDataArray(1,4) = 0.0
              fetchDataArray(1,5) = 0.0
              fetchDataArray(1,6) = 0.0
              allocate(fetchIndexArray(1))
              fetchIndexArray(1) = iindex
          end if

      enddo
    enddo

    call mui_announce_span_rcv(coord_min_rcvX,  &
                               coord_min_rcvY,  &
                               coord_min_rcvZ,  &
                               coord_max_rcvX,  &
                               coord_max_rcvY,  &
    ...
    if ((coord_min_rcvX > coord_max_rcvX).or.(coord_min_rcvY > coord_max_rcvY).or.(coord_min_rcvZ > coord_max_rcvZ)) then

        local_fetch = .false.

    endif

        ! Create an object of type muiCouplingFixedRelaxation
        fr = muiCouplingFixedRelaxation(3, cont, init_und_relx_coupling, get_MPI_Comm())

        ! Create an object of type muiCouplingAitken
        ak = muiCouplingAitken(3, cont, init_und_relx_coupling, get_MPI_Comm())

        ! Create an object of type muiCouplingIQNILS
        IQNILS = muiCouplingIQNILS(3, cont, init_und_relx_coupling, get_MPI_Comm())
        ...
        select case (muiCouplingMethod)
            ...
            case (2)
                ...
                ! Initialize fr
                call fr%initialize(3, cont, init_und_relx_coupling, get_MPI_Comm())

            case (3)

                ! Initialize ak
                call ak%initialize(0)

            case (4)

                ! Initialize IQNILS
                call IQNILS%initialize(3, cont, und_relx_coupling_Max, aitkenIterationN_IQNILS)
        ...
        if ((irangp.eq.0).or.(irangp.eq.-1)) then
            write(nfecra, *) "sub-iteration of MUI Coupling is: ", sub_iteration_MUI_Coupling, &
                " Under relaxation factor of MUI Coupling is: ", init_und_relx_coupling
        endif
        ...
        select case (muiCouplingMethod)
            ...
            case (2)
                ...
                write(nfecra, *) "{CS} coupling point size from FR coupling Method is: ", fr%pointSize()

            case (3)

                write(nfecra, *) "{CS} coupling point size from Aitken coupling Method is: ", ak%pointSize()

            case (4)

                write(nfecra, *) "{CS} coupling point size from IQNILS coupling Method is: ", IQNILS%pointSize()
...
call tridim(itrale, nvar, nscal, dt, sub_iteration_MUI_Coupling, &
                nbsubcpl, init_und_relx_coupling, push_force_MUI, &
                push_in_C, fr, ak, IQNILS, muiCouplingMethod)
