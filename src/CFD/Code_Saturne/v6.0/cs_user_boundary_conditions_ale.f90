...
subroutine usalcl &
 ( itrale ,                                                       &
   ...
   dt     ,                                                       &
   rcodcl , xyzno0 , disale , insubiter,    &
   fr, ak, IQNILS)
...
real(kind=8) disale(3,nnod), xyzno0(3,nnod)
...
double precision :: delta, deltaa, deltaaa
double precision :: dis_x, dis_y, dis_z
double precision :: reat_t_store = 0.d0
save reat_t_store
double precision :: toll = 0.001d0
type(muiCouplingFixedRelaxation) :: fr
type(muiCouplingAitken) :: ak
type(muiCouplingIQNILS) :: IQNILS
type(c_ptr) :: c_uniface, c_spatial, c_temporal
!double precision, external :: cs_fetch_disp_MUI_Coupling
...
double precision tFetchStart, tFetchEnd, tFLStart, tFLEnd
...
if (ttcabs.eq.0.0d0) then
   ...
   ! --- prescribe Flexible Beam a Dirichlet boundary with imposed displacement
    ...
    do ilelt = 1, nlelt
      ...
      ! index number of nodfac array corresponding to ifac
      !
      ! elemental pure integer function mesh::ipnfbr (integer, intent(in) ifac)
      ! position of the first node of the each boundary face in the array returned by nodfbr
      ! 
      ...
      do ii = ipnfbr(ifac), ipnfbr(ifac+1)-1 !! index number of nodfac array corresponding to ifac
        ...
        if (impale(inod).eq.0) then
            ...
            disale(1,inod) = dis_x   ! displacement /x
            disale(2,inod) = dis_y   ! displacement /y
            disale(3,inod) = dis_z   ! displacement /z
    ...
    CALL GETFBR('2 or 6 or 7',NLELT,LSTELT)
    !==========
    ...
    CALL GETFBR( '3 or 4 or 5',NLELT,LSTELT)
    !==========
    ...
    if ((ABS(ttcabs-reat_t_store).gt.toll).and.((irangp.eq.0).or.(irangp.eq.-1))) then
     write(nfecra, *) "{CS} Real Time= ", ttcabs, " [s]"
     reat_t_store = ttcabs
    endif

    if ((irangp.eq.0).or.(irangp.eq.-1)) then
     write(nfecra, *) "{CS} sub-iteration: ", insubiter
    endif

   totalIter = ((ntcabs-1) * sub_iteration_MUI_Coupling + insubiter)
            ...
            do ilelt = 1, nlelt
              ...
              do ii = ipnfbr(ifac), ipnfbr(ifac+1)-1 !! index number of nodfac array corresponding to ifac
                ...
                if (impale(inod).eq.0) then
                    ...
                    call cs_FSI_Coupling_Fetch(xyzno0(1, inod),       &
                                               xyzno0(2, inod),       &
                                               xyzno0(3, inod),       &
                                               insubiter, &
                                               disale(1,inod), &
    ...
    if (local_fetch) then
        if ((muiCouplingMethod .ne. 1) .and. (muiCouplingMethod .ne. 2)) then
            ...
            call cs_FSI_Coupling_Process ( insubiter, &
                                            fr, &
                                            ak, &
                                            IQNILS )
        ...
        do ilelt = 1, nlelt
          ...
          do ii = ipnfbr(ifac), ipnfbr(ifac+1)-1 !! index number of nodfac array corresponding to ifac
            ...
            if (impale(inod).eq.0) then
                ...
                disale(1,inod) = cs_FSI_Coupling_getXDeltaDisp(xyzno0(1, inod),       &
                                                               xyzno0(2, inod),       &
                                                               xyzno0(3, inod),       &
                                                               insubiter, &
                                                               disale(1,inod), &
                                                               ...
                                                               ak, &
                                                               IQNILS)
                disale(2,inod) = cs_FSI_Coupling_getYDeltaDisp(xyzno0(1, inod),       &
                                                               xyzno0(2, inod),       &
                                                               xyzno0(3, inod),       &
                                                               insubiter, &
                                                               disale(2,inod), &
                                                               ...
                                                               ak, &
                                                               IQNILS)
                disale(3,inod) = cs_FSI_Coupling_getZDeltaDisp(xyzno0(1, inod),       &
                                                               xyzno0(2, inod),       &
                                                               xyzno0(3, inod),       &
                                                               insubiter, &
                                                               disale(3,inod), &
