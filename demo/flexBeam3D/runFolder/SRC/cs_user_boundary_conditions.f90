...
if (ttcabs.le.(T)) then
  Umax = Umax*(1.0d0 - cos(pi*ttcabs/T))/2.0d0
else if (ttcabs.gt.(T)) then
  Umax = Umax
endif

write(nfecra, *) "Umax= ", Umax

! --- Prescribe at boundary faces of color 8 a wall; Flexible Beam
...
do ilelt = 1, nlelt
  ...
  itypfb(ifac) = iparoi ! Wall

enddo

! --- For boundary faces of color 4, assign an inlet; Inlet Boundary
...
do ilelt = 1, nlelt
  ...
  itypfb(ifac) = ientre ! Inlet

  do ii = ipnfbr(ifac), ipnfbr(ifac+1)-1
    inod = nodfbr(ii)
    x_vel = (Umax*xyznod(2, inod)*(0.4d0-xyznod(2, inod))*((0.4d0**2d0) &
                          -((xyznod(3, inod))**2d0))/((0.2d0**2d0)*(0.4d0**2d0)))

    rcodcl(ifac,iu,1) = x_vel ! inflow velocity x-axis
    rcodcl(ifac,iv,1) = 0.  ! inflow velocity y-axis
    rcodcl(ifac,iw,1) = 0.  ! inflow velocity z-axis

  enddo

enddo

! --- For boundary faces of color 5, assign an outlet; Outlet Boundary
...
do ilelt = 1, nlelt
  ...
  itypfb(ifac) = isolib  ! Outlet

enddo

! --- Prescribe at boundary faces of color 7 a wall; Upper Boundary
...
! --- Prescribe at boundary faces of color 3 a wall; Lower Boundary
...
! --- Prescribe at boundary faces of color 6 a symmetry; Front Boundary
...
do ilelt = 1, nlelt
  ...
  itypfb(ifac) = isymet ! Symmetries

enddo

! --- Prescribe at boundary faces of color 2 a wall; Back Boundary
