...
integer          iel
double precision x_criteria, y_criteria, z_criteria, x_cen, y_cen, z_cen, x_dist, y_dist, z_dist
double precision viscmx(ncelet)
...
if (iand(idftnp, ISOTROPIC_DIFFUSION).ne.0) then
  ...
  do iel = 1, ncel
    x_dist = xyzcen(1,iel)-x_cen
    y_dist = xyzcen(2,iel)-y_cen
    z_dist = xyzcen(3,iel)-z_cen

    if ((ABS(x_dist).lt.(x_criteria)).and.(ABS(y_dist).lt.(y_criteria)).and.(ABS(z_dist).lt.(z_criteria))) then
